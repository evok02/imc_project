...
class EditorAPI(Resource):
    ...
    @newspaper_ns.marshal_list_with(editor_model, envelope='editor')
    def get(self):
        return Agency.get_instance().get_editors()
    

    @newspaper_ns.doc(editor_model, description="Add a new editor")
    @newspaper_ns.expect(editor_model, validate=True)
    @newspaper_ns.marshal_with(editor_model, envelope='newspaper')
    def post(self):
